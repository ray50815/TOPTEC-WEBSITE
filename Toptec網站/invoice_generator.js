// v4 變更摘要：
// - StyleUp 的求解邏輯改為與 An Xin (USD) 完全一致 → 採用 findQuantitiesAnXinUSD。
// - 保留 v3 的修正：WIN TEAM 不再只出 Accessories、零數量不加分、非 JC 排序優先活躍品項數、
//   預設發票日期 -5 天與 INV.No 同步。
...
const companyConfigs = {
  ...
  'styleup': {
    ...
    // ★ 改為與 An Xin USD 相同的求解器
    findQuantities: findQuantitiesAnXinUSD,
    extraInvoiceLines: () => `<tr class="h-4"><td class="no-border"></td><td class="no-border"></td><td class="no-border"></td><td class="no-border"></td></tr>`,
  },
  'converge': {
...
// 零數量不加分
function roundnessScore(q){ if(q <= 0) return 0; if(q%1000===0) return 4; if(q%100===0) return 3; if(q%10===0) return 2; if(q%5===0) return 1; return 0; }
...
function basicRoundEvenSort(solutions){
  ...
  const activeTypes = (s)=> (s.q1>0) + (s.q2>0) + (s.q3>0);
  return solutions.slice().sort((A,B)=>{
    const aActive=activeTypes(A), bActive=activeTypes(B);
...
function findQuantitiesWinTeam(totalAmount){
  ...
  // 若存在含主機的解，優先保留之
  const withMain = solutions.filter(s=> (s.q1>0 || s.q2>0));
  if (withMain.length>0) solutions = withMain;
...
// （保留舊的 StyleUp 函式，但已不使用）
function findQuantitiesStyleUp(totalAmount){
  const companyKey = document.getElementById('company').value;
  const config = companyConfigs[companyKey];
  const prices = config.products.map(p=>p.price);
  const priceSumCents = Math.round((prices[0]+prices[1]+prices[2]) * 100);
  let amountCents = Math.round(totalAmount * 100);
  const makeSolutions = (amtCents)=>{
    if(amtCents>0 && amtCents % priceSumCents === 0){
      const q = amtCents / priceSumCents; if(q>0) return [{q1:q, q2:q, q3:q}];
    }
    return [];
  };
  let solutions = makeSolutions(amountCents);
  if(solutions.length>0){ return scoreAndSortSolutionsForConfig(config, solutions, totalAmount); }
  const adjustedCents = Math.ceil(amountCents / priceSumCents) * priceSumCents;
  if(adjustedCents !== amountCents){
    const statusDiv = document.getElementById('status');
    const adjustedAmount = adjustedCents / 100;
    statusDiv.innerHTML = `<span class=\"font-bold\">注意：</span>您輸入的金額 ${ (amountCents/100).toLocaleString() } 無法整除。<br>已自動<span class=\"font-bold\">向上調整</span>為最接近的有效金額 <span class=\"font-bold\">${ adjustedAmount.toLocaleString() }</span> 並生成發票。`;
    statusDiv.className='mt-4 text-center text-orange-600 font-semibold';
    solutions = makeSolutions(adjustedCents);
    solutions.forEach(s=> s.adjustedAmount = adjustedAmount);
    return scoreAndSortSolutionsForConfig(config, solutions, adjustedAmount);
  }
  return [];
